...
class SensorSoilMoistureTemp:
    ...
    @staticmethod
    def reset_address_to_default(port="/dev/ttyS2", baudrate=9600):
        """
        Resets the sensor's Modbus address to the factory default (0x01) 
        using the broadcast address (0xFE)[cite: 49].
        WARNING: Ensure ONLY ONE sensor is connected to the RS485 bus during this operation.
        """
        print("Attempting to reset sensor address to 0x01 using Broadcast Address (0xFE)...")
        # Initialize Modbus with broadcast address 0xFE (254 in decimal)
        modbus = Modbus_Film69(port=port, slaveaddress=254, baudrate=baudrate)
        
        # Command to set address to 0x01 using Broadcast (FE 06 02 00 00 01) [cite: 44]
        hex_cmd = "FE 06 02 00 00 01"
        try:
            # We expect no response, but we set a read length just in case the library requires it
            response, _ = modbus.send(hex_cmd, resopne_len=8, ID=254)
            print(f"Reset command sent. Sensor response: {response}")
        except Exception as e:
            # It is normal to trigger a timeout or error here because broadcast usually has no reply
            print(f"Reset command sent. (Note: Sensors typically do not reply to broadcast commands). Details: {e}")
        finally:
            modbus.close()
            print("-" * 50)
            print("IMPORTANT: You MUST disconnect and reconnect the power to the sensor for the new address to take effect.")
