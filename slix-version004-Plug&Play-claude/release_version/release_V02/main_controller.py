...
- Log Interceptor & Rotating File Handler
- Dynamic Configuration via JSON
- Single MQTT Connection with Dynamic Topic Switching
"""
...
class SmartFarmController:
    ...
    def _read_sensor_data(self, s_type, instance):
        ...
        elif s_type in ["soil_ec", "soil_ph"]: return instance.read_data() 
    ...
    # --- Worker Thread 1: Network Check Loop ---
        ...
        while self.running:
            ...
            if current_status != self.internet_available:
                ...
                if self.internet_available:
                    ...
                    if not self.tb.connected: self.tb.connect()
                else:
                    print("[Network] Internet connection lost.")
            
            time.sleep(self.net_interval)

    # --- Worker Thread 2: Sensor Reading Loop ---
        ...
        while self.running:
            ...
            for port, s_info in self.active_sensors.items():
                ...
                if data:
                    ...
                    print(f"Port {port} ({s_type}) Read Failed")

            time.sleep(self.read_interval)

    # --- Worker Thread 3: Telemetry Sending Loop ---
    def _telemetry_send_loop(self):
        while self.running:
            if not self.latest_data:
                print("[Telemetry Task] No new data to send.")
            else:
                print(f"\n[Telemetry Task] Sending data to ThingsBoard...")
                data_snapshot = list(self.latest_data.items())
                self.latest_data.clear()
                
                for port, data in data_snapshot:
                    if not self.running: break
    ...
    def send_io_status(self):
        if not self.tb or not self.tb.connected: return
    ...
    def send_telemetry_data(self, port, sensor_data, s_info):
        ...
        for key, value in sensor_data.items():
            if key in MEASUREMENT_NAMES.get(s_type, {}):
                ...
                instance_num = s_info.get('instance', '01')
                msg_name = f"{self.box_id}_{name_suffix}_{model}-{instance_num}"
                messages[msg_name] = [{"ts": ts_now, "values": {"data_value": value}}]
                
        if messages:
            if self.tb.send_telemetry(messages):
                print(f"[ThingsBoard] Sent Telemetry for Port {port}")

    def run(self):
        print("Smart Farm Controller is starting (Multithreaded Mode)...")
        
        self.t_network = threading.Thread(target=self._network_check_loop, daemon=True, name="Network-Thread")
        self.t_sensor = threading.Thread(target=self._sensor_reading_loop, daemon=True, name="Sensor-Thread")
        self.t_telemetry = threading.Thread(target=self._telemetry_send_loop, daemon=True, name="Telemetry-Thread")
    ...
    def stop(self):
        ...
        try:
            ...
            for s_info in self.active_sensors.values():
                ...
                if sensor_obj and hasattr(sensor_obj, 'close'): sensor_obj.close()
        except Exception as e:
