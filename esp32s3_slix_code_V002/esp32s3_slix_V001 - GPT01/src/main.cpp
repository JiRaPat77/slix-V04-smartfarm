...
// ── 12h ON / 5s OFF Power Cycle (จาก Def board) ──────────────────────────
...
void restart_loop() {
  ...
  while (true) {
    ...
    for (unsigned long remaining = ON_SECONDS; remaining > 0; ) {
      ...
      if (now_ms - lastNet >= NET_CHECK_MS) {
        lastNet = now_ms;
        checkNet();
      }

      // ── อ่าน sensor ทุก 5 วิ ───────────────────────────────────────
      static unsigned long lastRead = 0;
      if (now_ms - lastRead >= READ_INTERVAL_MS) {
        lastRead = now_ms;
        step2_read();
      }

      // ── ส่ง ThingsBoard ทุก 60 วิ ──────────────────────────────────
      static unsigned long lastSend = 0;
      if (now_ms - lastSend >= SEND_INTERVAL_MS) {
        lastSend = now_ms;
        uint64_t ts_ms = utcNowMs();
        step3_send(ts_ms);
    ...
    for (int i = 0; i < PORT_CONFIG_COUNT; i++) {
      sensor_en_set(PORT_CONFIG[i].port, false);
    }
...
void setup() {
  ...
  Serial.println("\n[SYS]  Ready — entering 12h ON / 5s OFF power cycle");
